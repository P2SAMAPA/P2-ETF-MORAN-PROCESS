...
from moran_process import moran_process_scores
...
def run_for_window(returns, window_days):
    ...
    raw_scores = moran_process_scores(ret_window, pop_size=config.POPULATION_SIZE, steps=config.SIMULATION_STEPS)
    norm_scores = normalize_scores(raw_scores)
    sorted_norm = sorted(norm_scores.items(), key=lambda x: x[1], reverse=True)
    top_etfs = [{"ticker": t, "moran_score_norm": s, "raw_score": raw_scores[t]} for t, s in sorted_norm[:config.TOP_N]]
    return {
        "window": window_days,
        "top_etfs": top_etfs,
...
def main():
    ...
    out_file = f"output/moran_process_{timestamp}.json"
