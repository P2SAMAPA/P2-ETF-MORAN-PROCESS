...
st.set_page_config(page_title="Moran Process – Evolutionary Game Engine", layout="wide")
...
st.markdown('<h1 style="text-align: center;">🧬 Moran Process – Evolutionary Game Engine</h1>', unsafe_allow_html=True)
st.markdown('<p style="text-align: center;">ETF competition as population genetics | Survival probability as evolutionary fitness</p>', unsafe_allow_html=True)

st.sidebar.markdown("## 🧬 Moran Process")
...
st.sidebar.markdown(f"**Population:** {config.POPULATION_SIZE} | **Steps:** {config.SIMULATION_STEPS}")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'moran_process_' in f]
    if not json_files:
        return None
...
def display_universe(universe_name, uni_data, window_data, window_label):
    ...
    for idx, etf in enumerate(top3):
        with cols[idx]:
            ...
            <div class="hero-card">
                <h3>{etf['ticker']}</h3>
                <p>Survival probability: {etf['moran_score_norm']:.3f}</p>
                <p style="font-size:0.9rem;">raw: {etf['raw_score']:.4f}</p>
            </div>
            """, unsafe_allow_html=True)
    with st.expander(f"Full ranking for {universe_name}"):
        df_full = pd.DataFrame(list(norm_scores.items()), columns=["Ticker", "Normalized Survival"])
        df_full["Raw Score"] = df_full["Ticker"].apply(lambda t: raw_scores[t])
        df_full = df_full.sort_values("Normalized Survival", ascending=False)
        st.dataframe(df_full, use_container_width=True)

tab1, tab2 = st.tabs(["📊 Best Window (Auto)", "🔍 Choose Window (Manual)"])

with tab1:
    st.header("🧬 Top ETFs by Evolutionary Survival Probability (Auto Best Window)")
    with st.expander("📖 Interpretation", expanded=False):
        st.markdown("""
        - **Moran process** (population genetics) models competition between types (ETFs) in a finite population.
        - Fitness = average historical return over the rolling window.
        - At each step, an ETF reproduces proportionally to its fitness, and a random ETF dies.
        - After many steps, the population reaches a stationary distribution.
        - The score is the **final proportion** of the population – a measure of evolutionary fitness.
        - Higher score → the ETF is "fitter" and likely to dominate – a trend‑following signal.
        """)
...
with tab2:
    ...
    st.markdown("Choose a rolling window to inspect the survival probabilities.")
    for universe_name, uni_data in data["universes"].items():
        if not uni_data or not uni_data.get("all_windows"):
            st.warning(f"No window data for {universe_name}")
            continue
...
st.sidebar.markdown("---")
st.sidebar.caption("Moran Process | Evolutionary game theory for ETF ranking")
